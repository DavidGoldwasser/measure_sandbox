...
# start the measure
class DavidGoldwasserHelloMeasure < OpenStudio::Measure::ModelMeasure
  # human readable name
  def name
    return "David Goldwasser Hello Measure"
  end
  ...
  def run(model, runner, user_arguments)
    ...
    model.getBuilding.setName("Building with #{new_space.name}")
    runner.registerInfo("Renamed the building to #{model.getBuilding.name}")
...
DavidGoldwasserHelloMeasure.new.registerWithApplication
